...
// Helper to build chapter content
function makeChapter({ id, title, hikmetliTitle, shortDesc, imageUrl, fenConcept, tefekkurInsight, tableRows, conclusion }) {
  const content = `
# ${hikmetliTitle}

## Mukaddime: Kainat Kitabından Bir Tefekkür Sayfası

Varlık alemi, kör tesadüflerin veya şuursuz tabiatın eseri değil; sonsuz bir ilim, kudret ve hikmet sahibinin her an yarattığı muazzam bir kitaptır. Fen bilimlerinin incelediği kanunlar, aslında bu ilahî nizamın ve adetullahın tecellilerinden ibarettir. **${title}** konusu da kainat sarayındaki bu kusursuz mimariyi ve hassas dengeyi açıkça gözler önüne sermektedir.

${imageUrl ? `![${hikmetliTitle}](${imageUrl})
fenbilim.net alıntıdır. [Resim Link](${imageUrl})
> **Görsel Tefekkür Yorumu:** İncelediğimiz bu görsel, zerrelerden kürelere kadar her varlığın kör tesadüflerle değil; şaşmaz bir hendese, ölçü ve merhametle tanzim edildiğini gösteren somut bir şahittir.
` : ''}

## Fen Bilgisi Özeti ve Hakikat-i Eşya

${fenConcept}

> **Hikmet Penceresi:** ${tefekkurInsight}

## Fen & Hikmet Karşılaştırması

| Fen Bilimleri Bakışı (Madde & Sebep) | Hikmetli Fen Bakışı (Kudret & Tevhid) | Tefekkür ve Basiret Çıkarımı |
| :--- | :--- | :--- |
${tableRows.map(r => `| ${r[0]} | ${r[1]} | ${r[2]} |`).join('\n')}

## Hülasa ve Basiret Notu
